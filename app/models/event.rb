class Event < ApplicationRecord
  ...
  scope :searchevents_for_feed, -> (authenticated_user_id) { where("rsvps.user_id = ? and rsvps.rsvp_status in (?) and start_at >= ?",
                                                      authenticated_user_id, [Rsvp.rsvp_statuses[:yes]], Time.zone.now)
                                                      .select('events.*, rsvps.rsvp_status as status') }
